...
import { getLocations, createOffer, createRequest } from '../src/services/marketplace';
import { Location } from '../src/lib/supabase';
      ...
      <main className="flex-1 p-6 space-y-8 pb-32">
          ...
          <div className="flex flex-wrap gap-2">
            {locations.map((loc) => (
              <button
                ...
                className={`flex h-10 items-center justify-center px-4 rounded-xl font-medium transition-all shadow-sm ${selectedLocation === loc.id ? 'bg-primary text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600 hover:border-primary'}`}
              >
                {loc.name}
              </button>
            ))}
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-bold leading-tight tracking-tight">Meal Date</h3>
          </div>
          <input
            type="date"
            value={mealDate}
            ...
            className="w-full bg-white border border-slate-200 rounded-xl h-12 px-4 text-slate-900 focus:ring-2 focus:ring-primary focus:border-primary"
          />
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div
              ...
              className={`flex flex-col items-center p-3 rounded-xl transition-all cursor-pointer ${mealType === 'breakfast' ? 'border-2 border-primary bg-primary/5 shadow-sm' : 'border border-slate-200 bg-white hover:border-primary'}`}
            >
              <span className={`material-symbols-outlined text-2xl mb-1 ${mealType === 'breakfast' ? 'text-primary' : 'text-slate-400'}`}>light_mode</span>
              <span className={`text-sm font-semibold ${mealType === 'breakfast' ? 'text-primary' : ''}`}>Breakfast</span>
            </div>
            <div
              onClick={() => setMealType('lunch')}
              className={`flex flex-col items-center p-3 rounded-xl transition-all cursor-pointer ${mealType === 'lunch' ? 'border-2 border-primary bg-primary/5 shadow-sm' : 'border border-slate-200 bg-white hover:border-primary'}`}
            >
              <span className={`material-symbols-outlined text-2xl mb-1 ${mealType === 'lunch' ? 'text-primary' : 'text-slate-400'}`}>sunny</span>
              <span className={`text-sm font-semibold ${mealType === 'lunch' ? 'text-primary' : ''}`}>Lunch</span>
            </div>
            <div
              onClick={() => setMealType('dinner')}
              className={`flex flex-col items-center p-3 rounded-xl transition-all cursor-pointer ${mealType === 'dinner' ? 'border-2 border-primary bg-primary/5 shadow-sm' : 'border border-slate-200 bg-white hover:border-primary'}`}
            >
              <span className={`material-symbols-outlined text-2xl mb-1 ${mealType === 'dinner' ? 'text-primary' : 'text-slate-400'}`}>dark_mode</span>
              <span className={`text-sm font-semibold ${mealType === 'dinner' ? 'text-primary' : ''}`}>Dinner</span>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-amber-accent/10 text-amber-accent px-3 py-1 rounded-full text-xs font-bold border border-amber-accent/20">
              Recommended: {selectedLocationData?.default_price || 1.0}
            </div>
          </div>
        ...
        <div className="p-4 bg-primary/5 rounded-xl border border-primary/10 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-900 font-semibold">
            {offerType === 'GIVE' ? 'Giving' : 'Requesting'} 1 <span className="text-primary font-bold">{mealType.charAt(0).toUpperCase() + mealType.slice(1)}</span> at <span className="text-primary font-bold">{selectedLocationData?.name || 'Location'}</span> for <span className="text-primary font-bold">{tokens} Tokens</span>
          </p>
        </div>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 p-4 pb-8 z-50 flex justify-center">
        <div className="w-full max-w-[480px]">
          <button
            ...
            disabled={loading}
            className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-primary/30 transition-all active:scale-95 disabled:opacity-50"
          >
            <span>{loading ? 'Posting...' : (offerType === 'GIVE' ? 'Post Offer' : 'Submit Request')}</span>
